    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [plData, bsData] = await Promise.all([
                accountingService.getProfitLoss(parseInt(activeCompanyId!), fromDate, toDate),
                accountingService.getBalanceSheet(parseInt(activeCompanyId!))
            ]);
...
<div onClick={() => navigate('/accounting/balance-sheet')} className="glass-card rounded-xl p-lg flex flex-col justify-between hover:shadow-md transition-shadow group cursor-pointer relative overflow-hidden">
...
<span className="text-[10px] uppercase tracking-wider text-on-surface-variant font-bold">Total Assets</span>
<span className="font-data-mono text-data-mono text-primary">₹ {balanceSheet?.total_assets?.toLocaleString('en-IN') || '0.00'}</span>
</div>
...
<div onClick={() => navigate('/accounting/profit-loss')} className="glass-card rounded-xl p-lg flex flex-col justify-between hover:shadow-md transition-shadow group cursor-pointer relative overflow-hidden">
<div className="relative z-10">
...
<div className="mt-lg pt-md border-t border-outline-variant/30 flex items-center justify-between relative z-10">
<div className="flex flex-col">
<span className="text-[10px] uppercase tracking-wider text-on-surface-variant font-bold">Net Profit</span>
<span className="font-data-mono text-data-mono text-primary">₹ {profitLoss?.net_profit?.toLocaleString('en-IN') || '0.00'}</span>
</div>
<button className="w-10 h-10 rounded-full bg-surface-container-highest flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-on-primary transition-all duration-300">
<span className="material-symbols-outlined">arrow_forward</span>
</button>
...
<div onClick={() => navigate('/accounting/trial-balance')} className="glass-card rounded-xl p-lg flex flex-col justify-between hover:shadow-md transition-shadow group cursor-pointer relative overflow-hidden">
<div className="relative z-10">
...
<div className="mt-lg pt-md border-t border-outline-variant/30 flex items-center justify-between relative z-10">
<div className="flex flex-col">
<span className="text-[10px] uppercase tracking-wider text-on-surface-variant font-bold">Ledgers</span>
